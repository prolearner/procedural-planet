...
modelSphere(geometry, terrains, raughness);
...
function simplex(coord){
  ...
  var t0 = 0.5 - dx0*dx0 - dy0*dy0 - dz0*dz0;
    ...
    var noise = 7*(n0 + n1 + n2 + n3);
    //if(noise < 0 || noise > 1)console.log(noise);
    return noise;
}
...
function simplexGradient(coord){
  ...
  var t0 = 0.5 - dx0*dx0 - dy0*dy0 - dz0*dz0;
    ...
    var gNoise = ((new THREE.Vector3(0,0,0)).add(g0).add(g1).add(g2).add(g3)).multiplyScalar(7);
    return gNoise;
}
...
function helper(x, y, z, resolution) {
  ...
  var interpolated = simplex(new THREE.Vector3(x, y, z));
  //var interpolated = tricosine(new THREE.Vector3(x, y, z));
  return interpolated;
}
...
function getNormal( position, smoothness, scale, radius, roughness){
...
function getHeight( position, smoothness, scale, cliff){
  var cliff = cliff || [0.5, 1.0] // [position, multiplyFactor]
  var scale = scale || 512;
  var lacunarity = 2.0;
  var nOctaves = 16;
  ...
  w.normalize();
  var height = null;
  if(noise=="RPN" || noise =="RSN") height = RidgedNoise(w.x ,w.y ,w.z, scale, smoothness, lacunarity, nOctaves, cliff[0], cliff[1]);
  else  height = PerlinNoise(w.x ,w.y ,w.z, scale*2.0, smoothness, lacunarity, nOctaves, cliff[0], cliff[1]);
  return height;
}


function perturbVector(v, smoothness, roughness, scale, cliff){
  var h = getHeight(v, smoothness, scale, cliff);
  var w = (new THREE.Vector3()).copy(v);
...
function perturbGeometry(geometry, smoothness, roughness, scale, cliff){
  amp = null;
  var roughness = roughness || 1.0;
  var smoothness = smoothness || 2.0;
  for(i in geometry.vertices){
     var v = geometry.vertices[i];
     perturbVector(v, smoothness, roughness, scale, cliff);
  }
}
